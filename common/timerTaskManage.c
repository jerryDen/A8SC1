...
void timerTask(int startTimems,pThreadFunc taskFunc,void *funcArg)
{
	struct sigevent hard_evp;
	timer_t timerid;
	struct itimerspec it;
	...
	clock_gettime(CLOCK_MONOTONIC, &current_time);
		//180S内只能报警一次
	if ((current_time.tv_sec - last_time.tv_sec)<(startTimems/1000))
	{
			return -1;
	}
	...
	memset(&hard_evp, 0, sizeof(struct sigevent));
	hard_evp.sigev_value.sival_ptr = funcArg;
    hard_evp.sigev_notify = SIGEV_THREAD;
    hard_evp.sigev_notify_function = taskFunc;
    hard_evp.sigev_notify_attributes = NULL;//&attr;
    if (timer_create(CLOCK_MONOTONIC, &hard_evp, &(timerid)) == -1)
    {  
        printf("fail to timer_create");
        return ;
    }
	it.it_value.tv_sec =  (startTimems/1000);
	if(startTimems <= 0) {
		it.it_value.tv_nsec = 1;
	}else
	    it.it_value.tv_nsec = (startTimems%1000)*1000*1000LL;
