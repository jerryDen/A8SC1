                                 ...
                                 AIO_ATTR_S* pstAioAttr, AUDIO_SAMPLE_RATE_E enInSampleRate, HI_BOOL bResampleEn, HI_VOID* pstAoVqeAttr, HI_U32 u32AoVqeType)
{
	...
	/*s32Ret = HI_MPI_AO_EnableChn(AoDevId, 1);
        if (HI_SUCCESS != s32Ret)
        {
            printf("%s: HI_MPI_AO_EnableChn(1)failed with %#x!\n", __FUNCTION__,s32Ret);
            return HI_FAILURE;
        }
	printf("%s: HI_MPI_AO_EnableChn(1)ok with %#x!\n", __FUNCTION__,s32Ret);
*/
