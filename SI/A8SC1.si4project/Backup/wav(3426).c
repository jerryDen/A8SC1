...
static void *playWavThread(void * arg)
{
	...
	if(us_wavInitPbPcm(wavParames.format,wavParames.channels,wavParames.vol))
			goto end;
...
start:
	...
	while(wavThreadID&&wavThreadID->check(wavThreadID) == Thread_Run){
		...
		if( loaded > chunk_bytes ) {
			if( us_wavWritePcm(wav_pb_buf + written, chunk_bytes ) < 0) 
				goto end;
			written += chunk_bytes;
			loaded -= chunk_bytes;
			continue;
		...
		if(us_wavWritePcm( wav_pb_buf, loaded)<0 )
				goto end;
		written += loaded;
		loaded = 0;
