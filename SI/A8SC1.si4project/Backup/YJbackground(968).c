...
static  int login(struct YJbackgroundOps* ops)
{
	...
	if(server == NULL)
		return -1;
	char *url  = "http://api.yunjiangkj.com/appVilla/login?";
	char  appver[12] = {0};
	char  md5sourceStr[64] = {0};
	char  macaddres[36] = {0};
	char  timestamp[24] = {0};
	...
	char *urltest =  "http://ml.yunjiangky.com";
	//char *urltest =  "https://www.baidu.com";
//	printf("getparameter:%s\n",getparameter);


//	server->httpClient->download("https://apps.sfcdn.org/apk/com.molitv.android.v2.674c7c7502ccee58ce381929ac98f9ad.apk?response-content-type=application%2fvnd.android.package-archive","./test.apk");
	//server->httpClient->download("https://www.yunjiangkj.com/yjufile/callshoot/201808/22/a6931aa3a6cf484fb7740f00a16f7621.jpg","test.jpg");	
//	return 0;
...
//	printf("pagesize:%d pagesize = %dserver->device.communityId = %d\n",pagesize,pageno,server->device.communityId);
	sprintf(lockid,"%d",(int)server->device.lockid);
	getfkey(lockid,timestamp,fkey);
	
	sprintf(getparameter,"%sFKEY=%s&TIMESTAMP=%s&LOCKID=%d&COMMUNITYID=%d&PAGENO=%d&PAGESIZE=%d",url,fkey,timestamp,(int)server->device.lockid,server->device.communityId,pageno,pagesize);
//	printf("getparameter:%s\n",getparameter);
	ret = server->httpClient->getUrl(getparameter,jsonStr,sizeof(jsonStr)-1);
	if(ret < 0)
	{
...
//	printf("postJson:%s\n",postJson);
	server->httpClient->postJson(url,postJson,jsonStr,sizeof(jsonStr));
...
pYJbackgroundOps createYJbackgroundServer(void)
{
	...
	if(server ==  NULL)
		goto fail0;
	bzero(server,sizeof(YJbackgroundServer));
	server->httpClient = getHttpClientServer();
	...
	server->ops = ops;
	return (pYJbackgroundOps)server;
	fail1:
		free(server);
		server = NULL;
	fail0:
